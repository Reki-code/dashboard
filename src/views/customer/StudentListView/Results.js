...
import { ALL_STUDENT } from '../../../graphql/user'
...
const columns = [
  ...
  { field: 'username', headerName: '学号', width: 140 },
...
const Results = ({ filter }) => {
  ...
  const stuentInfo = useQuery(ALL_STUDENT)
  const [updateUser] = useMutation(UPDATE_USER)
  const [deleteUser] = useMutation(DELETE_USER)

  if (stuentInfo.loading) return <div>Loading</div>
  if (stuentInfo.error) return <div>Error</div>

  const students = stuentInfo.data?.users
  ...
  const handleSave = (user) => {
    ...
    updateUser({
      ...
      update(cache, { data: { updateUser }}) {
        const dataInStore = cache.readQuery({ query: ALL_STUDENT })
  ...
  return (
    <>
          ...
          <div style={{ flexGrow: 1 }}>
            <DataGrid
              ...
              onSelectionModelChange={({ selectionModel }) => {
                setUser(students.find(student => student.id === selectionModel[0]))
              }}
            />
          </div>
        </div>
      ...
      >
        <DialogTitle id="alert-dialog-title">{"删除学生"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            {`确定要删除学生${user?.displayName}(${user?.username})吗？`}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
